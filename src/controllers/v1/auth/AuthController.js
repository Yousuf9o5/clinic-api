...
const signup = (req, res) => {
  ...
  user.save((err, user) => {
    if (err) {
      return res.status(500).json(error(500, `Server Side Error ${err}`));
    }
    ...
    if (roles) {
      Role.find(
        ...
        (err, roles) => {
          ...
          return res
            .status(201)
            .json(error(201, user, "Registered Successfully"));
        }
      );
    } else {
      // default is `user` // admin
      Role.findOne(
        {
          name: "user",
        },
        (e, role) => {
          if (e) return res.status(500).json(error(500, "Server Side Error"));
